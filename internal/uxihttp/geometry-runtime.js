(() => {
  ...
  style.textContent = `
    ...
    #geometryTruth{position:fixed;z-index:12;right:max(14px,env(safe-area-inset-right));top:50%;transform:translateY(-50%);width:min(278px,calc(100vw - 28px));padding:13px 14px;border:1px solid rgba(126,231,220,.16);border-radius:18px;background:rgba(4,10,18,.66);backdrop-filter:blur(16px);box-shadow:0 18px 45px rgba(0,0,0,.28);color:#eaf6ff;font:11px/1.45 Inter,system-ui,sans-serif;transition:opacity .2s,transform .2s}
    #geometryTruth[hidden]{display:none}
    #geometryTruth .geometry-kicker{color:#72e7d7;font-size:8px;letter-spacing:.18em;text-transform:uppercase}
    #geometryTruth h3{margin:4px 0 2px;color:#f5dfa8;font-size:17px;line-height:1.1}
    ...
    #geometryTruth a{display:block;margin-top:10px;padding:9px 11px;border-radius:999px;background:linear-gradient(90deg,rgba(85,234,219,.18),rgba(184,147,255,.18));color:#dffbf7;text-align:center;text-decoration:none;font-size:9px;font-weight:800;letter-spacing:.05em}
    #geometryTruth button{position:absolute;right:8px;top:7px;border:0;background:transparent;color:#8498b1;font-size:18px}
    #geometryLegend{position:fixed;z-index:11;left:50%;bottom:max(12px,env(safe-area-inset-bottom));transform:translateX(-50%);padding:6px 10px;border-radius:999px;background:rgba(3,8,15,.55);backdrop-filter:blur(10px);color:#8195ac;font:8px/1.2 Inter,system-ui,sans-serif;letter-spacing:.08em;pointer-events:none;white-space:nowrap}
  ...
  function drawTorus(node, point, flow) {
    ...
    ctx.rotate(flow);
    ctx.strokeStyle = color(node, state.selected === node ? .9 : .34);
    ctx.lineWidth = state.selected === node ? 2.2 : 1;
    ctx.shadowColor = color(node, .7);
  ...
  function render(now) {
    ...
    for (let i = 1; i < points.length; i++) {
      ...
      ctx.strokeStyle = "rgba(104,202,211,.075)";
      ctx.beginPath();
