...
echo "== Fix introspection kernel script =="

cat > phase-16-introspection-kernel.sh <<'SH'
#!/usr/bin/env sh
set -eu
...
chmod +x phase-16-introspection-kernel.sh

mkdir -p AI-Code-Training/scripts/phase-scripts
cp phase-16-introspection-kernel.sh AI-Code-Training/scripts/phase-scripts/phase-16-introspection-kernel.sh

echo "== Run fixed introspection kernel =="
sh phase-16-introspection-kernel.sh || true

echo "== AIFT-OS verify if available =="
if [ -x "$ROOT/aift" ]; then
  "$ROOT/aift" verify || true
fi
...
if git diff --cached --quiet; then
  ...
  git commit -m "Fix introspection kernel diagnostics"
fi
...
for repo in "$ROOT"/*; do
  ...
  if git diff --cached --quiet; then
    ...
    git commit -m "Sync federation diagnostics and phase updates"
  fi
