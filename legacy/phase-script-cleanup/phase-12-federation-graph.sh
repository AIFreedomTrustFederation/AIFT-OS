...
echo "== Phase 12: Federation Knowledge Graph Engine =="
...
cat > internal/graph/graph.go <<'GO'
package graph
...
cat > schemas/graph.schema.json <<'JSON'
{
  "$schema": "https://json-schema.org/draft/2020-12/schema",
  "title": "AIFT Federation Graph",
  "type": "object",
  "required": ["generatedAt", "nodes", "edges"],
  "properties": {
...
cat > schemas/node.schema.json <<'JSON'
{
  "$schema": "https://json-schema.org/draft/2020-12/schema",
  "title": "AIFT Graph Node",
  "type": "object",
  "required": ["id", "type", "name", "status", "evidence"],
  "properties": {
    "id": { "type": "string" },
    "type": { "type": "string" },
    "name": { "type": "string" },
...
cat > schemas/edge.schema.json <<'JSON'
{
  "$schema": "https://json-schema.org/draft/2020-12/schema",
  "title": "AIFT Graph Edge",
  "type": "object",
...
chmod +x tests/graph-smoke.sh

cat > docs/PHASE-12-FEDERATION-GRAPH.md <<'DOC'
...
cp phase-12-federation-graph.sh AI-Code-Training/scripts/phase-scripts/phase-12-federation-graph.sh 2>/dev/null || true
...
"$ROOT/aift" graph
"$ROOT/aift" graph summary
"$ROOT/aift" graph repo AIFT-OS
sh tests/graph-smoke.sh
...
if git diff --cached --quiet; then
  ...
  git commit -m "Add federation knowledge graph engine"
fi
...
echo "DONE."
echo "Try:"
echo "  ~/AIFT/aift graph summary"
echo "  ~/AIFT/aift graph repo booksmith-ai"
echo "  ~/AIFT/aift graph type Capability"
echo "  ~/AIFT/aift graph status planned"
