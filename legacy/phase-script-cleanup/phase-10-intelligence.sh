...
echo "== AIFT-OS Phase 10: Real Federation Intelligence Layer =="
...
cat > internal/intelligence/intelligence.go <<'GO'
...
if contains(ri.DetectedFiles, "README.md") { score += 10; confidence += 5 }
if contains(ri.DetectedFiles, ".aift/repo.json") { score += 10; confidence += 10 }
if contains(ri.DetectedFiles, ".aift/capabilities.json") { score += 10; confidence += 10 }
if len(ri.Ready) > 0 { score += len(ri.Ready) * 8; confidence += 10 }
if len(ri.V1) > 0 { score += len(ri.V1) * 12; confidence += 15 }
if len(ri.Detected) > 0 { score += len(ri.Detected) * 3 }
if len(ri.Broken) > 0 { score -= len(ri.Broken) * 10; confidence += 10 }
if len(ri.Frameworks) > 0 { score += 5; confidence += 5 }
if ri.Remote != "" { score += 5 }
if ri.Dirty { score -= 5 }
...
if '"github.com/AIFreedomTrustFederation/AIFT-OS/internal/intelligence"' not in s:
    s = s.replace(
        '"github.com/AIFreedomTrustFederation/AIFT-OS/internal/gitx"',
        '"github.com/AIFreedomTrustFederation/AIFT-OS/internal/gitx"\n\t"github.com/AIFreedomTrustFederation/AIFT-OS/internal/intelligence"'
    )

if 'fmt.Println("  intelligence scan|report|repo|roadmap")' not in s:
    s = s.replace(
        'fmt.Println("  capabilities scan|report|repo|promote")',
        'fmt.Println("  capabilities scan|report|repo|promote")\n\tfmt.Println("  intelligence scan|report|repo|roadmap")'
    )

case_block = r'''case "intelligence":
err = runIntelligence(cfg, args)
'''

if 'case "intelligence":' not in s:
...
cat > tests/intelligence-smoke.sh <<'SH'
#!/usr/bin/env sh
set -eu
...
SH
chmod +x tests/intelligence-smoke.sh

cat > docs/PHASE-10-INTELLIGENCE.md <<'DOC'
# Phase 10: Real Federation Intelligence

The intelligence layer is based on real repository state only.
...
- actual manifests
- actual capability records
- actual detected frameworks
- actual generated reports

It does not simulate maturity. It reports confidence, score, role, recommendations, and orchestration readiness from evidence.

Commands:

- `aift intelligence scan`
- `aift intelligence report`
- `aift intelligence repo <repo>`
- `aift intelligence roadmap`

Generated files:

- `registry/intelligence.json`
- `reports/intelligence.md`
DOC

cp phase-10-intelligence.sh AI-Code-Training/scripts/phase-scripts/phase-10-intelligence.sh 2>/dev/null || true
...
"$ROOT/aift" intelligence scan
"$ROOT/aift" intelligence repo AIFT-OS
"$ROOT/aift" intelligence roadmap
sh tests/intelligence-smoke.sh
...
if git diff --cached --quiet; then
  ...
  git commit -m "Add real federation intelligence layer"
fi
...
echo "DONE."
echo "Try:"
echo "  ~/AIFT/aift intelligence report"
echo "  ~/AIFT/aift intelligence repo AIFT-Forge"
echo "  ~/AIFT/aift intelligence roadmap"
