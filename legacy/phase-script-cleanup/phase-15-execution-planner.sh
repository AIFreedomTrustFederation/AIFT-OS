...
echo "== Phase 15: Federation Execution Planner =="

mkdir -p internal/planner docs tests registry reports schemas var/events AI-Code-Training/scripts/phase-scripts bin

cat > internal/planner/planner.go <<'GO'
package planner
...
cat > schemas/execution-plan.schema.json <<'JSON'
{
  "$schema": "https://json-schema.org/draft/2020-12/schema",
  "title": "AIFT Federation Execution Plan",
  "type": "object",
  "required": ["generatedAt", "summary", "repos"],
  "properties": {
...
chmod +x tests/planner-smoke.sh

cat > docs/PHASE-15-EXECUTION-PLANNER.md <<'DOC'
...
cp phase-15-execution-planner.sh AI-Code-Training/scripts/phase-scripts/phase-15-execution-planner.sh 2>/dev/null || true
...
"$ROOT/aift" plan build
"$ROOT/aift" plan summary
"$ROOT/aift" plan repo AIFT-OS
sh tests/planner-smoke.sh
...
if git diff --cached --quiet; then
  ...
  git commit -m "Add federation execution planner"
fi
...
echo "DONE."
echo "Try:"
echo "  ~/AIFT/aift plan summary"
echo "  ~/AIFT/aift plan ready"
echo "  ~/AIFT/aift plan blocked"
echo "  ~/AIFT/aift plan report"
