...
function TorusIcon({ size = 60 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 60 60" fill="none">
      <ellipse cx="30" cy="30" rx="26" ry="26" stroke="rgba(255,255,255,0.15)" strokeWidth="1" />
      <ellipse cx="30" cy="30" rx="18" ry="18" stroke="rgba(255,255,255,0.10)" strokeWidth="0.8" />
      <ellipse cx="30" cy="30" rx="8"  ry="8"  stroke="rgba(255,255,255,0.08)" strokeWidth="0.6" />
      <ellipse cx="30" cy="30" rx="26" ry="10" stroke="rgba(0,221,136,0.25)"   strokeWidth="1" />
      <ellipse cx="30" cy="30" rx="18" ry="7"  stroke="rgba(0,221,136,0.15)"   strokeWidth="0.7" />
    </svg>
  );
}
...
function FlowerOfLifeIcon({ size = 60 }: { size?: number }) {
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 60 60" fill="none">
      <circle cx={cx} cy={cy} r={r} stroke="rgba(255,200,50,0.2)" strokeWidth="0.7" />
      {angles.map((a, i) => (
        <circle
          key={i}
          cx={cx + Math.cos(a) * r}
      ...
      <circle cx={cx} cy={cy} r={r * 2} stroke="rgba(255,200,50,0.08)" strokeWidth="0.5" />
    </svg>
  );
}
...
function SpiralIcon({ size = 60 }: { size?: number }) {
  ...
  const path = "M30,30 Q35,25 38,30 Q41,37 35,42 Q27,47 20,40 Q11,30 18,20 Q27,8 40,15 Q52,23 50,38";
  return (
    <svg width={size} height={size} viewBox="0 0 60 60" fill="none">
      <path d={path} stroke="rgba(153,34,255,0.3)" strokeWidth="1" fill="none" />
        ...
        >
          {SACRED_GEOMETRY.map(({ label, sublabel, Icon }) => (
            <div key={label} className="flex flex-col items-center gap-2">
              ...
              <span className="font-mono text-[8px] text-white/15">{sublabel}</span>
            </div>
          ))}
        </motion.div>
        ...
        >
          ...
          <div className="max-w-3xl mx-auto mb-6">
            <p className="text-white/60 font-mono text-sm leading-relaxed mb-4">
              Spacetime, light, consciousness, information, and causality are different expressions of one continuous, self-folding topology.
            </p>
            <p className="text-white/50 font-mono text-sm leading-relaxed mb-4">
              The long-term objective is not another software platform. The objective is a truthful digital ecosystem where every sovereign installation contributes to a larger federation without surrendering ownership of its own knowledge.
            </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-w-2xl mx-auto mb-14 text-left">
            {[
              ...
              >
                ...
                <p className="font-mono text-[10px] text-white/50 leading-relaxed">{p}</p>
              </div>
            ))}
          </div>
          ...
          <a
            ...
            style={{
              background: "linear-gradient(135deg, rgba(255,200,50,0.15), rgba(255,180,20,0.08))",
              border: "1px solid rgba(255,200,50,0.35)",
              color: "#e8c040",
              boxShadow: "0 0 30px rgba(255,200,50,0.1)",
            }}
            onMouseEnter={(e) => {
              (e.currentTarget as HTMLElement).style.boxShadow = "0 0 50px rgba(255,200,50,0.2)";
            }}
            onMouseLeave={(e) => {
              (e.currentTarget as HTMLElement).style.boxShadow = "0 0 30px rgba(255,200,50,0.1)";
            }}
          >
