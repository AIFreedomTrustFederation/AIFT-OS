...
echo "== Phase 20: Federation Bootstrap =="
...
mkdir -p "$CMD_DIR" "$REPORT_DIR" "$REGISTRY_DIR" "$BOOTSTRAP_DIR"

REPORT="$REPORT_DIR/phase20-federation-bootstrap-report.md"
REGISTRY="$REGISTRY_DIR/commands.json"
BOOTSTRAP="$BOOTSTRAP_DIR/federation-bootstrap.json"

cp "$CMD_DIR/main.go" "$CMD_DIR/main.go.phase20.bak" 2>/dev/null || true
...
func collectChecks(root string) []Check {
checks := []Check{
...
git add "$CMD_DIR/main.go" "$REGISTRY" "$BOOTSTRAP" "$REPORT" phase20-federation-bootstrap.sh || true
