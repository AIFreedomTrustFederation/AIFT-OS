...
echo "== AIFT-OS Phase 7: Runtime + Internal API =="
...
cat > internal/state/state.go <<'GO'
package state
...
cat > internal/services/services.go <<'GO'
package services
...
cat > internal/supervisor/supervisor.go <<'GO'
package supervisor
...
'case "events":\n\t\terr = events.Tail(cfg, 25)': 'case "events":\n\t\terr = events.Tail(cfg, 25)\n\tcase "services":\n\t\terr = services.List(cfg)\n\tcase "tick":\n\t\terr = runtime.Tick(cfg)',
'"github.com/AIFreedomTrustFederation/AIFT-OS/internal/runtime"': '"github.com/AIFreedomTrustFederation/AIFT-OS/internal/runtime"\n\t"github.com/AIFreedomTrustFederation/AIFT-OS/internal/services"',
}
...
cat > tests/runtime-api-smoke.sh <<'SH'
#!/usr/bin/env sh
set -eu
...
SH
chmod +x tests/runtime-api-smoke.sh

cat > docs/PHASE-7-RUNTIME-API.md <<'DOC'
...
if git diff --cached --quiet; then
  ...
  git commit -m "Add AIFT-OS federation runtime and internal API"
fi
